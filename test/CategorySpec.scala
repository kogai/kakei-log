...
class CategorySpec extends PlaySpec {
  "Models.Category" should {
    "Categoryテーブルからカテゴリーツリーを作れる" in {
      ...
      assert(expected.head.children.size == 6)
      assert(expected.head.children.head.name == "肉類")
      assert(expected.head.children(1).name == "魚介類")
      assert(expected(1).children.size == 1)
    }
  }
}
